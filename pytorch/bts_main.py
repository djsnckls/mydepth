...
parser.add_argument('--multiprocessing_distributed',           help='Use multi-processing distributed training to launch '
                                                                    ...
                                                                    'multi node data parallel training', action='store_true',) #不传入该参数则为False
# Online eval
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    var_sum = [var.sum().detach().cpu().numpy() for var in model.parameters() if var.requires_grad]
    var_cnt = len(var_sum)
...
def main():
    ...
    if args.checkpoint_path == '':
        ...
        # os.system(command)
        shutil.copy(loaded_model_dir + '/' + loaded_model_filename, model_out_path)
